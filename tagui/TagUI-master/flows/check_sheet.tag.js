...
var flow_path = 'D:/Tai_lieu_hoc/HK1Nam4/gd2/KDTM/BTH4/tagui/TagUI-master/flows';
...
chrome_step('Page.navigate',{url: 'https://docs.google.com/spreadsheets/d/1zmIibdE9M2L90FQWX9gsG-DO0cVKtaYndqWMouWrEzE/edit'}); sleep(1000);
techo('https://docs.google.com/spreadsheets/d/1zmIibdE9M2L90FQWX9gsG-DO0cVKtaYndqWMouWrEzE/edit' + ' - ' + chrome.getTitle() + '\n');});

casper.then(function() {techo('wait 10   // chờ load');});
casper.then(function() {casper.wait(10000, function() {});});

// TỪ A1 → F1
casper.then(function() {{techo('keyboard [right][right][right][right][right]   // di chuyển từ A→F'); var fs = require('fs');
if (!sikuli_step('keyboard [right][right][right][right][right]   // di chuyển từ A→F')) if (!fs.exists('[right][right][right][right][right]   // di chuyển từ A→F') && !false)
{this.echo('ERROR - cannot find image file [right][right][right][right][right]   // di chuyển từ A→F').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/Tai_lieu_hoc/HK1Nam4/gd2/KDTM/BTH4/tagui/TagUI-master/flows/check_sheet_error.png');
this.echo('ERROR - cannot find [right][right][right][right][right]   // di chuyển từ A→F on screen').exit(); this.wait(0);}}});

// NHẬP DỮ LIỆU
casper.then(function() {{techo('keyboard "Đang kiểm tra..."'); var fs = require('fs');
if (!sikuli_step('keyboard "Đang kiểm tra..."')) if (!fs.exists('"Đang kiểm tra..."') && !false)
{this.echo('ERROR - cannot find image file "Đang kiểm tra..."').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/Tai_lieu_hoc/HK1Nam4/gd2/KDTM/BTH4/tagui/TagUI-master/flows/check_sheet_error.png');
this.echo('ERROR - cannot find "Đang kiểm tra..." on screen').exit(); this.wait(0);}}});
...
casper.then(function() {techo('wait 0.3');});
casper.then(function() {casper.wait(300, function() {});});

// LẶP 10 DÒNG TIẾP THEO
...
casper.then(function() {this.echo('"✔ DONE - Đã ghi dữ liệu vào cột F!"');});
